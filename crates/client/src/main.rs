...
fn main() -> anyhow::Result<()> {
    // Handle helper subcommands (mcp, rename-conversation) before anything else.
    // These are invoked as subprocesses by the MCP machinery and must not run
    // the full client startup.
    if helper::dispatch_if_helper() {
        return Ok(());
    }
